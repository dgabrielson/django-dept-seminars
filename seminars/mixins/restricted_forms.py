...
class RestrictedAdminMixin(RestrictedBaseMixin):
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        ...
        field = super(RestrictedAdminMixin, self).formfield_for_dbfield(db_field, **kwargs)
        if request is not None and self._should_restrict(request.user):
            if db_field.name in self.restricted_foreign_key_fields:
                fk_restrict = {self.restricted_foreign_key_fields[db_field.name]: request.user}
    ...
    def get_prepopulated_fields(self, request, obj=None):
        ...
        """
        prepop_fields = super(RestrictedAdminMixin, self).get_prepopulated_fields(request, obj)
        if self._should_restrict(request.user):
    ...
    def get_form(self, *args, **kwargs):
        ...
        if self._should_restrict(self.request.user):
            ...
            for f in self.restricted_foreign_key_fields:
                if f in form.fields:
                    fk_restrict = {self.restricted_foreign_key_fields[f]: self.request.user}
                    form.fields[f].queryset = form.fields[f].queryset.filter(**fk_restrict)
